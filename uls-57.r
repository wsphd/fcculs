...
#  )
  ...
  # create indexes (tblGeoUS3)
  print( paste0( "indexing ", "tblGeoUS3", "..." ))
  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX call_sign_tblGeoUS3_index ON tblGeoUS3 (call_sign)" )
  ...
  # 4
  time.tic <- Sys.time()
  print( paste0( "generating the '", "tblGeoUS4 (tblGeoUS3<-->LO)", "' join", "..." ))
  s.rows <- dbExecute( conn = s4.db,
    "CREATE TABLE tblGeoUS4
       AS SELECT DISTINCT
        tblGeoUS3.call_sign                     ,
        tblGeoUS3.frequency_assigned            ,
        tblGeoUS3.class_station_code            ,
        tblGeoUS3.cnt_mobile_units              ,
        tblGeoUS3.unique_system_identifier      ,
        tblGeoUS3.frequency_upper_band          ,
        tblGeoUS3.frequency_carrier             ,

        tblGeoUS3.emission_code                 ,

        tblGeoUS3.state_code                    ,
        tblGeoUS3.control_county                ,
        tblGeoUS3.control_city                  ,
        tblGeoUS3.control_address               ,
        tblGeoUS3.control_phone                 ,

        tblGeoUS3.contact_licensee              ,
        tblGeoUS3.state                         ,
        tblGeoUS3.city                          ,
        tblGeoUS3.street_address                ,
     ...
     FROM tblGeoUS3
        ...
        INNER JOIN PUBACC_LO ON tblGeoUS3.call_sign=PUBACC_LO.call_sign

    "
  )

  # create indexes (tblGeoUS4)
  print( paste0( "indexing ", "tblGeoUS4", "..." ))
  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX call_sign_tblGeoUS4_index ON tblGeoUS4 (call_sign)" )

  print( Sys.time() - time.tic )


  s.rows <- dbExecute( conn = s4.db,
    "DROP TABLE tblGeoUS3
    "
  )

  s.rows <- dbExecute( conn = s4.db,
    "DROP TABLE tblGeoUS2
    "
  )

  s.rows <- dbExecute( conn = s4.db,
    "DROP TABLE tblGeoUS1
    "
  )
  ...
  # 5
  time.tic <- Sys.time()
  print( paste0( "generating the '", "tblGeoUS5 (tblGeoUS4<-->HD)", "' join", "..." ))
  s.rows <- dbExecute( conn = s4.db,
    "CREATE TABLE tblGeoUS5
       AS SELECT DISTINCT
        tblGeoUS4.call_sign                     ,
        tblGeoUS4.frequency_assigned            ,
        tblGeoUS4.class_station_code            ,
        tblGeoUS4.cnt_mobile_units              ,
        tblGeoUS4.unique_system_identifier      ,
        tblGeoUS4.frequency_upper_band          ,
        tblGeoUS4.frequency_carrier             ,

        tblGeoUS4.emission_code                 ,

        tblGeoUS4.state_code                    ,
        tblGeoUS4.control_county                ,
        tblGeoUS4.control_city                  ,
        tblGeoUS4.control_address               ,
        tblGeoUS4.control_phone                 ,

        tblGeoUS4.contact_licensee              ,
        tblGeoUS4.state                         ,
        tblGeoUS4.city                          ,
        tblGeoUS4.street_address                ,

        tblGeoUS4.location_state                ,
        tblGeoUS4.location_county               ,
        tblGeoUS4.location_city                 ,
        tblGeoUS4.location_address              ,
        tblGeoUS4.location_name                 ,
        tblGeoUS4.ground_elevation              ,
        tblGeoUS4.lat_degrees                   ,
        tblGeoUS4.lat_minutes                   ,
        tblGeoUS4.lat_seconds                   ,
        tblGeoUS4.lat_direction                 ,
        tblGeoUS4.long_degrees                  ,
        tblGeoUS4.long_minutes                  ,
        tblGeoUS4.long_seconds                  ,
        tblGeoUS4.long_direction                ,
     ...
     FROM tblGeoUS4
        ...
        INNER JOIN PUBACC_HD ON tblGeoUS4.call_sign=PUBACC_HD.call_sign

    "
  )

  # create indexes (tblGeoUS5)
  print( paste0( "indexing ", "tblGeoUS5", "..." ))
  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX call_sign_tblGeoUS5_index ON tblGeoUS5 (call_sign)" )
...
# make the final table
  time.tic <- Sys.time()
  print( paste0( "sorting the final '", "tblGeoUS", "' table", "..." ))
  s.rows <- dbExecute( conn = s4.db,
    "CREATE TABLE tblGeoUS
  AS SELECT DISTINCT
      tblGeoUS5.frequency_assigned       ,
      tblGeoUS5.call_sign                ,

      tblGeoUS5.license_status           ,
      tblGeoUS5.radio_service_code       ,

      tblGeoUS5.class_station_code       ,

      tblGeoUS5.location_state           ,
      tblGeoUS5.location_county          ,
      tblGeoUS5.location_city            ,
      tblGeoUS5.location_address         ,
      tblGeoUS5.location_name            ,

      tblGeoUS5.contact_licensee         ,
      tblGeoUS5.state                    ,
      tblGeoUS5.city                     ,
      tblGeoUS5.street_address           ,

      tblGeoUS5.state_code               ,
      tblGeoUS5.control_county           ,
      tblGeoUS5.control_city             ,
      tblGeoUS5.control_address          ,
      tblGeoUS5.control_phone            ,

      tblGeoUS5.cnt_mobile_units         ,

      tblGeoUS5.grant_date               ,
      tblGeoUS5.expired_date             ,
      tblGeoUS5.cancellation_date        ,
      tblGeoUS5.effective_date           ,
      tblGeoUS5.last_action_date         ,

      tblGeoUS5.ground_elevation         ,
      tblGeoUS5.lat_degrees              ,
      tblGeoUS5.lat_minutes              ,
      tblGeoUS5.lat_seconds              ,
      tblGeoUS5.lat_direction            ,
      tblGeoUS5.long_degrees             ,
      tblGeoUS5.long_minutes             ,
      tblGeoUS5.long_seconds             ,
      tblGeoUS5.long_direction           ,

      tblGeoUS5.emission_code            ,

      tblGeoUS5.frequency_upper_band     ,
      tblGeoUS5.frequency_carrier        

  FROM tblGeoUS5

  ORDER BY tblGeoUS5.frequency_assigned,
      tblGeoUS5.call_sign,
      tblGeoUS5.license_status,
      tblGeoUS5.radio_service_code,
      tblGeoUS5.class_station_code,
  ...
  s.rows <- dbExecute( conn = s4.db,
    "DROP TABLE tblGeoUS5
    "
  )

  s.rows <- dbExecute( conn = s4.db,
    "DROP TABLE tblGeoUS4
    "
  )
...
#  print( paste0( "indexing ", "..." ))
  ...
  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX call_sign_EM_index ON PUBACC_EM (call_sign, frequency_assigned)" )
...
#  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX emission_code2_EM_index ON PUBACC_EM (call_sign, emission_code)" )
  ...
  s.rows <- dbExecute( conn = s4.db, "CREATE INDEX call_sign_FR_index ON PUBACC_FR (call_sign, frequency_assigned)" )
  ...
  s.rows <- dbExecute( conn = s4.db,
    "UPDATE PUBACC_EN
      SET entity_name = UPPER( entity_name ),
       state = UPPER( state ),
       city = UPPER( city ),
       street_address = UPPER( street_address )
    "
  )
...
f.dbms <- function( v.zip.file.prefixes, v.data.file.prefixes, s.dbname ) {
  ...
  # imputation queries (not stable enough to use in production...ws)
  s.state <- "CA"
...
f.preprocessAFile <- function( s.zip.file.prefix, s.data.file.prefix ) {
  ...
  # fix each file (tbd...this should really only be done for the "CP" files--but add the DB_ID field too)
...
#  s.url.basepath <- "ftp://wirelessftp.fcc.gov/pub/uls/complete/"
  ...
  # the default timeout of 60 seconds isn't sufficient for the big FCC files
  # use the higher of 300 or the setting set by the user ("R_DEFAULT_INTERNET_TIMEOUT")
  options( timeout = max( 500, getOption( "timeout" )))

  # this is needed for home (non-VPN) use
  # doesn't work (at home, VPN, R v4.2)
#  options( download.file.method = "auto" )
  # works, but deprecated (at home, VPN, R v4.2)
#  options( download.file.method = "wininet" )

  options( download.file.method = "auto" )

# this section needs to be double-checked and reviewed periodically
if( Sys.info()[ "sysname" ] == "Windows" ) {
  download.file( url = s.url, destfile = s.zip.file.name )
} else if( Sys.info()[ "sysname" ] == "MacOS" ) {
  # works on MacOS
  system( paste0( "curl -O ", s.url ))
} else {
  # works on Linux and *BSD
  # wget doesn't work anymore on Nautilus (why? deprecated by the FCC? it doesn't like the user-agent header?)
#  system( paste0( "wget ", s.url ))
  # works
#  system( paste0( "curl -O ", s.url ))
  # works (but must send a non-wget user-agent header)
#  system( paste0( "wget -U Mozilla/5.0 ", s.url ))
  # works (but has to be custom installed onto a base Ubuntu image)
  system( paste0( "wget2 --progress=bar ", s.url ))
}
...
f.main <- function() {
  ...
  # capture the start of elapsed time *overall*
  time.tic.overall <- Sys.time()

  # print the elapsed time *overall*
  print( paste0( "The overall start time was ", time.tic.overall ))
...
#  s.dbname <- ":memory:"
  ...
  s.logfile <- "1.log" 
  if( !file.exists( s.logfile )) {
    f.getAllFiles( v.zip.file.prefixes, v.data.file.prefixes )
    con.file <- file( description = s.logfile, open = "w" )
    writeLines( text = "Done", con = con.file )
    close( con = con.file )
  }
  ...
  # delete the log files because all three parts have completed successfully
  s.logfile <- "1.log"
  if( file.exists( s.logfile )) {
    file.remove( s.logfile )
    unlink( x = s.logfile )
  }
  ...
  # print the elapsed time *overall*
  print( paste0( "The elapsed time overall was ", Sys.time() - time.tic.overall, " hours" ))
